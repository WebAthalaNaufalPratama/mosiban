...
if (isset($_POST['p'])) {
    ...
    elseif ($_POST['p'] == "profile") {
        ...
        $q = mysqli_query($link, "SELECT sed_sim, tanggal, waktu, curah_hujan, lokasi from curah_hujan where sn='$sn' order by id DESC limit 1");
        // echo "SELECT sed_sim, tanggal, waktu, curah_hujan, lokasi from curah_hujan where sn='$sn' order by id DESC limit 1";
        while ($d = mysqli_fetch_row($q)) {
            $sed_sim = $d[0];
            $tanggal = $d[1];
            $waktu = $d[2];
    ...
    if ($produk_tipe == 'realtime') { //buat data realtime dynamic chart 
        $q = mysqli_query($link, "SELECT sed_sim, tanggal, waktu, curah_hujan from curah_hujan where sn='$sn' order by id DESC limit 1");
        // echo "SELECT sed_sim, tanggal, waktu, curah_hujan from curah_hujan where sn='$sn' order by id DESC limit 1";
        $d = mysqli_fetch_row($q);
        $sed_sim = $d[0];
        $tanggal = $d[1];
        $waktu = $d[2];
        ...
        $qj = mysqli_query($link, "SELECT count(id) FROM curah_hujan where sn='$sn' ORDER BY id DESC LIMIT 1");
        $dj = mysqli_fetch_row($qj);
        $j = $dj[0];
        $j1 = $j - 20;
        //echo "j: $j, j1: $j1 --> SELECT tanggal,waktu,curah_hujan from curah_hujan order by id ASC limit $j1,$j";
        $q = mysqli_query($link, "SELECT tanggal,waktu,curah_hujan from curah_hujan where sn='$sn' order by id ASC limit $j1,$j");
        echo "SELECT tanggal,waktu,curah_hujan from curah_hujan where sn='$sn' order by id ASC limit $j1,$j";
        while ($d = mysqli_fetch_row($q)) {
        ...
        $j1 = $j - 20;
        //echo "j: $j, j1: $j1 --> SELECT tanggal,waktu,curah_hujan from curah_hujan order by id ASC limit $j1,$j";
        $q = mysqli_query($link, "SELECT tanggal,waktu,sed_sim from curah_hujan where sn='$sn' order by id ASC limit $j1,$j");
        while ($d = mysqli_fetch_row($q)) {
            //$wkt_ms=strtotime("$d[0]T$d[1]")*1000;
            $rows1['data'][] = [$d[1], $d[2]];
            // $rows1['data'][] = $d[1];
        ...
        if (empty($tgl2) || $tgl1 == $tgl2) {
            $data['name'] = 'Time';
            $q = mysqli_query($link, "SELECT waktu as t1 FROM curah_hujan WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2') AND HOUR(waktu) = '$waktu' group by UNIX_TIMESTAMP(waktu) DIV 300");
        ...
        if (empty($tgl2) || $tgl1 == $tgl2) {
            $rows['name'] = 'Time';
            if ($produk_tipe == "sed_sim") {
                $q = mysqli_query($link, "SELECT HOUR(waktu) as t1 FROM curah_hujan WHERE sn='$sn' AND tanggal='$tgl1' GROUP BY HOUR(waktu)");
                // echo "SELECT HOUR(waktu) as t1 FROM curah_hujan WHERE tanggal='$tgl1' GROUP BY HOUR(waktu)";
                $q2 = mysqli_query($link, "SELECT SUM($field_name) AS $field_name from curah_hujan where sn='$sn' AND tanggal ='$tgl1' group by HOUR(waktu)");
            ...
            $rows['name'] = 'Date';
            if ($produk_tipe == "sed_sim") {
                $q = mysqli_query($link, "SELECT tanggal as dr FROM curah_hujan WHERE sn='$sn' AND (tanggal BETWEEN '$tgl1' AND '$tgl2')  GROUP BY tanggal");
                // echo "SELECT tanggal as dr FROM curah_hujan WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2')  GROUP BY tanggal";
                $q2 = mysqli_query($link, "SELECT SUM($field_name)  AS $field_name from curah_hujan where sn='$sn' AND tanggal BETWEEN '$tgl1' AND '$tgl2' group by tanggal");
            } else if ($produk_tipe == "rain") {
                $q = mysqli_query($link, "SELECT tanggal as dr FROM curah_hujan WHERE sn='$sn' AND (tanggal BETWEEN '$tgl1' AND '$tgl2')  GROUP BY tanggal");
                // echo "SELECT tanggal as dr FROM curah_hujan WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2')  GROUP BY tanggal";
                $q2 = mysqli_query($link, "SELECT SUM($field_name)  AS $field_name from  curah_hujan where sn='$sn' AND tanggal BETWEEN '$tgl1' AND '$tgl2' group by tanggal");
